...
func (s *Service) validateBindTempAuthKey(ctx context.Context, sessionID int64, binding domain.TempAuthKeyBinding) (mtcrypto.BindAuthKeyInner, int, domain.TempAuthKeyBindingResult, error) {
	...
	// PostgreSQL proves the two protocol keys and commits their immutable
	// identity relation. v2 Layer state is authoritative in Redis and is merged
	// by LayerAwareTempAuthKeyBindingStore after the identity write succeeds.
	return inner, pair.Temporary.ExpiresAt, domain.TempAuthKeyBindingResult{}, nil
}
